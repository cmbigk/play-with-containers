...
#
# NOTE ON SECURITY: We use 'sudo -u vagrant' here because it’s better practice for
# each service to run as a non-privileged user rather than as root.
...
# Version where root owns the process list
# source /etc/profile.d/app_env.sh && pm2 delete $SERVICE_NAME 2>/dev/null || true
# source /etc/profile.d/app_env.sh && pm2 start $START_FILE --name $SERVICE_NAME --interpreter ./venv/bin/python --restart-delay 3000
# pm2 save
